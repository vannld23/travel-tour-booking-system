...
<head>
    ...
    <style>
        body { font-family: 'Inter', sans-serif; background-color: #f8f9f9; }
        .material-symbols-outlined { font-variation-settings: 'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24; }
        .tour-card { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
        .tour-card:hover { transform: translateY(-8px) scale(1.02); box-shadow: 0px 8px 24px rgba(0, 0, 0, 0.08); }
            ...
            <div class="flex items-center gap-4">
                <c:choose>
                    <c:when test="${not empty sessionScope.currentUser}">
                        <div class="flex items-center gap-4">
                            <span class="text-sm font-semibold text-deep-navy">Chào, ${sessionScope.currentUser.fullName}</span>
                            <c:if test="${sessionScope.currentUser.roleId == 1}">
                                <a href="<c:url value='/dashboard'/>" class="text-sm text-ocean-blue hover:underline font-semibold flex items-center gap-0.5">
                                    <span class="material-symbols-outlined text-sm">dashboard</span> Admin
                                </a>
                            </c:if>
                            <a href="<c:url value='/booking/history'/>" class="text-sm text-deep-navy hover:underline font-semibold">Lịch sử</a>
                            <a href="<c:url value='/auth/logout'/>" class="border border-outline text-on-surface px-4 py-1.5 rounded-lg text-sm font-semibold hover:bg-surface-container transition-all">Đăng xuất</a>
                        </div>
                    </c:when>
                    <c:otherwise>
                        <a href="<c:url value='/auth/login'/>" class="bg-ocean-blue text-white px-6 py-2 rounded-lg font-label-md hover:bg-primary transition-all active:scale-95 text-center">Sign In</a>
                    </c:otherwise>
                </c:choose>
            </div>
        </div>
